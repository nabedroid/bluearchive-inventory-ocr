...
export const toHsv = (src: any): any => {
  const dst = new cv.Mat();
  if (src.channels() === 4) {
    const rgbMat = new cv.Mat();
    cv.cvtColor(src, rgbMat, cv.COLOR_RGBA2RGB);
    cv.cvtColor(rgbMat, dst, cv.COLOR_RGB2HSV);
    rgbMat.delete();
 ...
 * Mat をグレースケールに変換する
 */
export const toGray = (src: any): any => {
  const dst = new cv.Mat();
  if (src.channels() === 4) {
...
/**
 * DataUrl から Mat を作成する
 * @param dataUrl DataUrl 文字列
 * @returns Mat オブジェクト
 */
export const fromDataUrl = (dataUrl: string): Promise<any> => {
  return new Promise((resolve, reject) => {
    const image = new Image();
    image.onload = () => {
      try {
