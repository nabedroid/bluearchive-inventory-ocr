"""
アセットバンドルからアイテム、装備マスターを作成する
アイテムを含めるとフロントの計算量が増えるので必要ないなら装備のみのmain.pyでマスターを作ること
"""
import base64
import cv2
import glob
...
from item_master import ITEM_MASTER

def iter_equipment_icons(base_path: str) -> Iterator[tuple[dict[str, dict], Image.Image]]:
  """アイテム、装備アイコンを読み込む"""
  # Texture名からアイテムマスタを検索できるようにする
  all_master = {equipment['filename']: equipment for equipment in EQUIP_MASTER}
  all_master.update({item['filename']: item for item in ITEM_MASTER})
  ...
  # アイテムのテクスチャが含まれるバンドル
  env.load_files(glob.glob(os.path.join(base_path, '*common*nonequipment*textures*.bundle')))
  for obj in env.objects:
    if obj.type.name != 'Texture2D': continue
    texture = obj.read()
    # マスターに未登録の Texture はスキップ
    if all_master.get(texture.m_Name) is None: continue
    yield all_master[texture.m_Name], texture.image
...
if __name__ == "__main__":
  ...
  with open(os.path.join(output_path, 'all-master.json'), 'w', encoding='utf-8') as f:
    json.dump(output_all_master, f, ensure_ascii=False, indent=2)
