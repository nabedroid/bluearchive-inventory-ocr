...
import { InventoryExtractionService, type AnalyzedItem } from './services/inventoryExtractionService'
...
function App() {
  ...
  const handleAnalyze = useCallback(async () => {
    ...
    try {
      ...
      const items = await inventoryExtractionService.extractAllAsync(files, settings, (percent, message) => {
        setProgress({ percent, message });
      })

      setResults(items);

      setProgress({ percent: 100, message: '解析完了！' });
    } catch (e: any) {
      console.error(e);
      setProgress({ percent: 0, message: `エラーが発生しました: ${e.message}` });
    } finally {
  ...
  return (
    <div className="app">
      <header className="app-header" style={{ padding: '20px', backgroundColor: '#ffffff', color: '#000000', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <h1 style={{ margin: 0 }}>ブルーアーカイブ インベントリ OCR</h1>
      ...
      <main className="app-main" style={{ padding: '20px' }}>
        <section className="upload-section">
          ...
          <div className="actions" style={{ marginTop: '15px' }}>
            ...
            >
              {isAnalyzing ? '解析中...' : `解析開始（${files.length}枚）`}
            </button>
          </div>
        </section>
